...
export const RegisterPage: React.FC = () => {
  ...
  const getStrengthColor = () => {
    ...
    if (strengthScore <= 50) return 'bg-yellow-500';
                ...
                <div className="mt-2">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className={`text-xs font-semibold ${
                      strengthScore <= 25 ? 'text-red-500' :
                      strengthScore <= 50 ? 'text-yellow-500' :
                      strengthScore <= 75 ? 'text-blue-500' :
                      'text-green-500'
                    }`}>{getStrengthLabel()}</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-1.5 flex overflow-hidden">
                    <div className={`${getStrengthColor()} h-1.5 transition-all duration-300`} style={{ width: `${Math.max(strengthScore, 5)}%` }}></div>
                  </div>
                </div>
              )}
            </div>
